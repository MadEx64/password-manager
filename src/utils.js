 ...
 *
 * @param {number} [userLength=0] - Optional. The desired password length (0 means random length).
...
export function decryptFile(encryptedData, masterPassword) {
  try {
    ...
    if (encryptedData.length < 69 + metadataLength) {
      throw new PasswordManagerError(red('Invalid encrypted file format (metadata size error)'), bold(red(ERROR_CODES.INVALID_ENCRYPTION_FORMAT)));
    }
